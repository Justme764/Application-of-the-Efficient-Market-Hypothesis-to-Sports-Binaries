...
class NBAVegasInsiderCrawler:
    ...
    def deploy(self, year, start_date, end_date):
        """
        ...
        while url_date != end_date:
            ...
            try:
                ...
                if len(game_tables) == 0:
                    print("No Games Scheduled on", url_date)
                    print('\n')
                    url_date = self.advance_date(url_date)
                    driver.close()
                    continue
            ...
            for game_table_index in range(len(game_tables)):
                ...
                try:
                    ...
                    line_movement_row = game_table.\
                        find_element_by_xpath('.//tr[@style="background: url(/graphics/component_shadow2.gif) repeat-x;"]')
        ...
        self.write_csv('NBA Vegas Moneyline (' + str(year) + ').csv', moneylines_matrix)
        self.write_csv('NBA Vegas Spreads (' + str(year) + ').csv', spreads_matrix)
    ...
    def advance_date(current_date):
        ...
        month_ids = list(MONTHS.values())
        if day == NUM_DAYS[months[month_ids.index(month)]]:
            next_day = str('01')
            if month == 12:
                next_month = str('01')
